...
def clean_raw_content(raw_text):   #remove extra brackets
    ...
    for i in range(max_len, 0, -1):   #backwards, chops off characters from the end  -> (start, stop, step) 
        try:
            firstChar = raw_text[:i]       #take first charactes, parse it as a json
...
#Deduplicate
dim_user = pd.DataFrame(list(set(user_email_records)), columns = ['email']) 
...
fact_df = fact_df.loc[:, ~fact_df.columns.duplicated()]   #remove duplicates;      : means all rows, ~ -> bitwise NOT --- 
                                                                                   #selects only non-duplicate columns
...
for _, row in raw_df.iterrows():   #
    try:
        ...
        for email in emails:
            ...
            bridge_rows.append({'communication_id': meeting_id, 'user_id': user_id})
    except:
        continue
